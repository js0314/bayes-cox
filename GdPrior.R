"GdPrior" <- function(X_train, time_train, event_train, KAP.j, xi_j, mc.size=3000){
  ...
  NEG.LOG.MAR.GD <- MAR.GD <- rep(NA, LL)
  ...
  for(GD.rep in 1:LL){
    ...
    for(i in 1:iter){
      ...
      for(j in out.threshold){
        ...
        Likehood <- function(b.j){ 
          ...
          (-1)*(loglik_num(X = X_train, time_train, event_train, hat.beta))+(-1)*dnorm(b.j, 0,1/sqrt(d_j),log=TRUE)
        }
        ini.b.j <- hat.beta[j]
        hat.beta[j] <- optim(ini.b.j, Likehood, method="BFGS")$par
    ...
    Post <- function(bb){
      ...
      (-1)*( loglik_num(X = as.matrix(X_train[,out.threshold]), time_train, event_train, bb)) + (-1)*sum(dnorm(bb, 0, 1/sqrt(dd_j), log=TRUE))
    }
    ...
    if(sum(abs(hat.beta))==0){
      ...
      Post.sam <- mvrnorm(mc.size, Post.mean, Post.var)
      log.g <- dmnorm(Post.sam, Post.mean, Post.var, log=TRUE)
      num_cal <- function(X, time, event, out.threshold, bb){
        loglik_num(X = as.matrix(X[,out.threshold]), time, event, bb) + sum(dnorm(bb, 0, 1/sqrt(length(out.threshold)), log=TRUE))
      }
      ...
      NEG.LOG.MAR.GD[GD.rep] <- (-1)*( log(sum(exp(log.like.fin.tmp - max.log) )) + max.log - log(nrow(X_train)))
  ...
  # Select Beta_map
  # sel.kap.id <- which.max(MAR.GD)
  sel.kap.id <- which.min(NEG.LOG.MAR.GD)
  
  return(list(
    beta.map = BETA.GD[sel.kap.id, ],
    d.map = D.GD[sel.kap.id, ],
    opt.k = KAP.j[sel.kap.id],
    opt.bic =  NEG.LOG.MAR.GD[sel.kap.id]
  ))
}
